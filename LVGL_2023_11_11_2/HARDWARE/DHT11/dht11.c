...
GPIO_InitStructure.GPIO_OType = GPIO_OType_PP; // 芢侺怀堤      //芢侺葩蚚怀堤
...
void DHT11_IO_IN (void){ //端口???入
	...
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP; // ゐ蚚奻嶺萇郯
	GPIO_Init(DHT11PORT, &GPIO_InitStructure);
}
...
u8 DHT11_Init (void){	//DHT11初始化
	RCC_AHB1PeriphResetCmd(RCC_AHB1Periph_GPIOA, ENABLE); //PA15	外???使能      
...
u8 DHT11_ReadData(u8 *temp){ //?取一次?据//?度值(十?制，范?:20%~90%) ，?度值(十?制，范?:0~50°)，返回值：0,正常;1,失? 
    u8 buf[5];
    u8 i;
    DHT11_RST();//DHT11端口复位，?出起始信?
    if(Dht11_Check()==0){ //等待DHT11回?
        for(i=0;i<5;i++){//?取5位?据
