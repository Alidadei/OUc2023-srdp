...
static lv_obj_t* tileview;		//创建平铺视图部件
...
///存储手机锁设定时间以及久坐提醒时间间隔的变量(单位：min)
uint8_t PhonelockTime=1;//手机锁存时间
uint8_t ReminderTime=1;//提醒时间间隔
...
void tile_page_set()
{
		...
		lv_obj_set_tile_id(tileview,0,0,LV_ANIM_OFF);//设置为主页面
}
...
static void switch3_event_cb(lv_event_t* e)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED)
    {
        if(lv_obj_has_state(target,LV_STATE_CHECKED))
        {
			...
			TIM_SetCompare1(TIM4,250);//同步数值到灯的亮度
        }
        else
        {
			...
			TIM_SetCompare1(TIM4,0);//同步数值到灯的亮度
        }
    }
}
...
static void switch4_event_cb(lv_event_t* e)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED)
    {
        if(lv_obj_has_state(target,LV_STATE_CHECKED))
        {
			...
			TIM_SetCompare2(TIM4,250);//同步数值到风扇转速
        }
        else
        {
            printf("%s","Fan OFF\n");
			...
			TIM_SetCompare2(TIM4,0);//同步数值到风扇转速
        }
    }
}
...
static void slider1_event_cb(lv_event_t* e)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED)//若值有变化
    {
		...
		TIM_SetCompare1(TIM4,(uint32_t)(((double)slider_value/100)*499));//更改占空比以调节亮度
        if(slider_value == 0)
        {
...
static void slider2_event_cb(lv_event_t* e)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED)//若值有变化
    {
		...
		TIM_SetCompare2(TIM4,(uint32_t)(((double)slider_value/100)*499));//更改占空比以调节风扇转速
        if(slider_value == 0)
        {
...
static void slider3_event_cb(lv_event_t* e)
{
    ...
    lv_obj_t *target = lv_event_get_target(e);//获取触发源
    int slider_value = lv_slider_get_value(target);//滑块当前数值
    if(code == LV_EVENT_VALUE_CHANGED)//若提醒时间设置值有变化
    {
        lv_label_set_text_fmt(slider3_label2,"%dmin",slider_value);//更新标签
		ReminderTime = slider_value;//更新提醒时间间隔记录变量（单位min）
        if(slider_value == 0)
        {
...
static void slider4_event_cb(lv_event_t* e)
{
    ...
    lv_obj_t *target = lv_event_get_target(e);//获取触发源
    int slider_value = lv_slider_get_value(target);//滑块当前数值
    if(code == LV_EVENT_VALUE_CHANGED)//若手机锁存时间值有变化
    {
        lv_label_set_text_fmt(slider4_label2,"%dmin",slider_value);//更新标签
		PhonelockTime = slider_value;//更新手机锁存时间记录变量（单位min）
        if(slider_value == 0)
        {
...
///温度亮度修改函数
void Temp_Bright_Update(uint8_t temp,u16 brightness)
{
...
void tileview_page1_set()
{
    ...
    lv_label_set_text_fmt(time_label1,"%d:%d:%d",19,27,23);//显示时分秒
    ...
    lv_label_set_text_fmt(time_label2,"20%d/%02d/%02d",23,11,11);//显示年月日
...
void tileview_page3_set()
{
    ...
    lv_obj_add_state(slider1,LV_STATE_CHECKED);//默认情况下为使能状态,直至开启开关
