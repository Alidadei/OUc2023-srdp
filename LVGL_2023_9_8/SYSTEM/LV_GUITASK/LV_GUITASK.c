...
static void switch1_event_cb(lv_event_t* e)
{
    ...
    if(code == LV_EVENT_VALUE_CHANGED)
    {
        if(lv_obj_has_state(target,LV_STATE_CHECKED))
        {
            ...
            lv_slider_set_value(slider3,60,LV_ANIM_OFF);//开启开关，将滑块值置默认值50
            lv_label_set_text_fmt(slider3_label2,"%d%%",60);//同时更改百分比标签
            lv_obj_clear_state(slider3,LV_STATE_DISABLED);//同时清除滑块1的失能状态
        }
        else
        {
            printf("%s","Adult Mode OFF\n");
            lv_slider_set_value(slider3,0,LV_ANIM_OFF);//关闭开关，同时将滑块值置0
            lv_label_set_text_fmt(slider3_label2,"%d%%",0);//同时更改百分比标签
            lv_obj_add_state(slider3,LV_STATE_DISABLED);//同时添加滑块1的失能状态
        }
    }
}
...
void tileview_page3_set()
{
    ...
    lv_obj_align(slider1,LV_ALIGN_CENTER,-5,-scr_act_width()/8);//设置滑块位置
    ...
    lv_obj_align(slider2,LV_ALIGN_CENTER,-5,scr_act_height()/16);//设置滑块位置
    ...
    lv_obj_align(slider3,LV_ALIGN_CENTER,-5,scr_act_height()/3-10);//设置滑块位置
    ...
    lv_label_set_text(slider3_label1,"Reminder Interval");//设置风扇转速文本标签
